...
**V1b is built and its gate is open.** `packages/engrave` engraves noteheads, stems, flags,
ledger lines, beams and accidentals from Bravura's own metrics, behind the same seam as the
VexFlow adapter. `pnpm proof nasty-chart --bar 6 --compare` is the demo. Findings and the
estimate — 5–6 focused days to parity, with spacing rather than beams as the risk — are in
`docs/v1b-engraver-spike.md`, which ends with four questions for Jian. **The full
replacement stays unscheduled until those are answered** (ADR-0030), and the engraver is
not wired into the PDF path: `pnpm render` still goes through VexFlow, unchanged.
...
Two adapters implement the seam now, and `tests/arch` holds both to it. One thing they
cannot agree on is where a bar's music starts, because layout computes the clef and key
signature's width and does not publish it — see `docs/v1b-engraver-spike.md`.
...
Its metrics are vendored, not derived. `packages/engrave/src/bravura.generated.ts` is
generated by `pnpm vendor:bravura` from the pinned `bravura-1.392` release and is **not
edited by hand**: fifteen glyph outlines plus `engravingDefaults`, cross-checked between
`bravura_metadata.json` and Bravura's own SVG font. Licensing is in
`packages/engrave/NOTICE.md` (SIL OFL 1.1).

The rule that follows: **no tuning constants.** A stem's attachment comes from the
notehead's `stemUpSE` anchor, a flag's from its `stemUpNW`, every thickness from
`engravingDefaults`. If you find yourself nudging a number to make a glyph look right, the
anchor you want probably exists — `anchor()` throws for one the font does not publish
rather than returning zero, on purpose.
...
| Full engraver replacement | awaiting the V1b gate | The spike has run and returned a number; the four questions in `docs/v1b-engraver-spike.md` are what schedule it (ADR-0030) |
