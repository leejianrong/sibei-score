...
/**
 * `sbscore export --pdf` (V3d), against a real server over real HTTP.
 *
...
beforeEach(async () => {
  ...
  work = mkdtempSync(join(tmpdir(), 'sbscore-export-'));
  out = [];
  err = [];
});
...
async function sbscoreWith(client: Client, ...argv: string[]): Promise<{ code: number; out: string }> {
  out = [];
  err = [];
  const code = await run(argv, {
    client,
...
describe('sbscore export writes a real PDF', () => {
  it('names the file after the chart and puts it in the working directory', async () => {
    await aChart();
    const result = await sbscore('export', 'soul');
  ...
  it('writes exactly where -o names a file', async () => {
    ...
    expect((await sbscore('export', 'soul', '-o', path)).code).toBe(EXIT.ok);
    expect(isPdf(path)).toBe(true);
  });
  ...
  it('puts the chart’s name inside -o when -o is a directory', async () => {
    ...
    expect((await sbscore('export', 'soul', '-o', existing)).code).toBe(EXIT.ok);
  ...
  it('says which path it could not write, rather than failing obscurely', async () => {
    await aChart();
    const result = await sbscore('export', 'soul', '-o', join(work, 'nowhere', 'chart.pdf'));
  ...
  it('--pdf is optional and means what the default means', async () => {
    await aChart();
    await sbscore('export', 'soul', '-o', join(work, 'implicit.pdf'));
    await sbscore('export', 'soul', '--pdf', '-o', join(work, 'explicit.pdf'));
  ...
  it('passes the paper and the face through, so they reach the page', async () => {
    ...
    await aChart();
    await sbscore('export', 'soul', '-o', join(work, 'a4-normal.pdf'));
    await sbscore('export', 'soul', '--paper', 'letter', '-o', join(work, 'letter.pdf'));
    await sbscore('export', 'soul', '--font', 'jazz', '-o', join(work, 'jazz.pdf'));
  ...
  it('serves the same bytes the second time, which is the cache doing its job', async () => {
    ...
    await sbscore('export', 'soul', '-o', join(work, 'second.pdf'));
...
describe('--json, because an agent should never parse prose', () => {
  it('reports the path and the byte count, and never the bytes', async () => {
    await aChart();
    const result = await sbscore('export', 'soul', '--json');
    expect(result.code).toBe(EXIT.ok);
...
describe('the exit codes hold for export too (ADR-0008)', () => {
  it('2 for a paper this build cannot produce, with the list of what it can', async () => {
    await aChart();
    const result = await sbscore('export', 'soul', '--paper', 'a5');
  ...
  it('2 for a face it does not have, with the list one level down in --json', async () => {
    await aChart();
    const result = await sbscore('export', 'soul', '--font', 'swing', '--json');
    expect(result.code).toBe(EXIT.validation);
  ...
  it('5 for a score that is not there, and nothing is written', async () => {
    expect((await sbscore('export', 'nope')).code).toBe(EXIT.notFound);
    expect(readdirSync(work)).toEqual([]);
  });

  it('1 for usage: no id, and -o with nothing after it', async () => {
    expect((await sbscore('export')).code).toBe(EXIT.usage);
    const dangling = await sbscore('export', 'soul', '-o');
...
describe('a filename that came off a socket is not a path', () => {
  ...
  it('cannot climb out of the directory the caller chose', async () => {
    ...
    const result = await sbscoreWith(serverSaying('../../../pwned.pdf'), 'export', 'soul', '-o', directory);
  ...
  it('cannot become an absolute path', async () => {
    ...
    await sbscoreWith(serverSaying(target), 'export', 'soul', '-o', directory);
  ...
  it('falls back to a name of its own when the suggestion is not a filename at all', async () => {
    ...
    for (const hostile of ['', '..', '.', '../../../', 'evil\u0000.pdf']) {
      ...
      const result = await sbscoreWith(serverSaying(hostile), 'export', 'soul', '-o', directory);
      expect(result.code).toBe(EXIT.ok);
  ...
  it('sanitises a score id before using it as a name, because that is user text too', async () => {
    const directory = mkdtempSync(join(work, 'chosen-'));
    await sbscoreWith(serverSaying(''), 'export', '../../etc/passwd', '-o', directory);
