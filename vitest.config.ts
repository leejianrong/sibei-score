 ...
 * due bill since V1.
 *
 *   fast   No infra. Pure logic, the engraver, the render path, the arch assertions. This is
 *          the layer the pre-push hook runs, and it has to stay quick enough that nobody is
 *          ever tempted by --no-verify: a slow gate gets bypassed, and then it protects
 *          nothing.
...
export default defineConfig({
  test: {
    projects: [
      { test: { name: 'fast', include: FAST, environment: 'node', testTimeout: TIMEOUT } },
